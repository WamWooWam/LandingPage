...
const recentVideos = async (req: Request, res: Response) => {
    ...
    for (const video of items) {
        ...
        content
            .getElementsByTagName('image')[0]
            .setAttribute('alt', 'Thumbnail for ' + video.snippet.title);
        content.getElementsByTagName('text')[0].textContent =
            video.snippet.title;

        rootElement.appendChild(
            root.importNode(content.getElementsByTagName('visual')[0], true),
        );

        let wideContent = TileUpdateManager.getTemplateContent(
        ...
        wideContent
            ...
            .setAttribute('src', channel.snippet.thumbnails.medium.url);
        wideContent
            .getElementsByTagName('image')[1]
            .setAttribute('alt', 'Thumbnail for ' + channel.snippet.title);
        wideContent.getElementsByTagName('text')[0].textContent =
            video.snippet.videoOwnerChannelTitle;
        wideContent.getElementsByTagName('text')[1].textContent =
            video.snippet.title;

        rootElement.appendChild(
            root.importNode(
                wideContent.getElementsByTagName('visual')[0],
                true,
            ),
        );
    }
