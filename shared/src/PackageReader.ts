...
export class PackageReader {
    ...
    async readPackage(): Promise<Package> {
        ...
        const manifestDocument = this.parser.parseFromString(
        ...
        const identityElement = manifestDocument.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'Identity',
        )[0]!;
        const propertiesElement = manifestDocument.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'Properties',
        )[0]!;
        const applicationsElement = manifestDocument.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'Applications',
        )[0]!;
        const osMinVersion = manifestDocument.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'OSMinVersion',
        )[0]?.textContent;
        ...
        this.identity = pack.identity = this.loadTextResources(
            await this.readIdentity(identityElement),
        );

        console.log(
            `loading package ${this.identity.packageFamilyName} with mode ${PackageCompatibilityMode[this.compatibilityMode]}`,
        );

        pack.properties = this.loadTextResources(
            this.readProperties(propertiesElement),
        );

        const applicationElements = applicationsElement.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'Application',
        );
        for (let i = 0; i < applicationElements.length; i++) {
            const applicationElement = applicationElements[i];
            const application = this.loadTextResources(
                this.readApplication(applicationElement),
            );
        ...
        const capabilitiesElement = manifestDocument.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'Capabilities',
        )[0];
    ...
    private async readIdentity(element: Element): Promise<PackageIdentity> {
        ...
        const hash = new Uint8Array(
            await crypto.subtle.digest('SHA-256', bufView),
        );
    ...
    private readProperties(element: Element): PackageProperties {
        const displayName = element.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'DisplayName',
        )[0]!.textContent!;
        const description = element.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'Description',
        )[0]?.textContent!;
        const publisherDisplayName = element.getElementsByTagNameNS(
            AppX2010ManifestNS,
            'PublisherDisplayName',
        )[0]!.textContent!;
        const logo = this.fixupUrl(
            element.getElementsByTagNameNS(AppX2010ManifestNS, 'Logo')[0]
                ?.textContent,
        )!;
    ...
    private readApplication(element: Element): PackageApplication {
        ...
        const visualElementsElement = element.getElementsByTagNameNS(
            AppX2013ManifestNS,
            'VisualElements',
        )[0]!;
        const visualElements = this.loadTextResources(
            this.readVisualElements(visualElementsElement),
        );
    ...
    private readVisualElements(element: Element): ApplicationVisualElements {
        ...
        visualElements.foregroundText = <ForegroundText>(
            element.getAttribute('ForegroundText')?.toLowerCase()
        );
        ...
        if (this.compatibilityMode === PackageCompatibilityMode.windows80) {
            visualElements.square150x150Logo = this.fixupUrl(
                element.getAttribute('Logo'),
            )!;
            visualElements.square30x30Logo = this.fixupUrl(
                element.getAttribute('SmallLogo'),
            )!;
        } else {
            visualElements.square150x150Logo = this.fixupUrl(
                element.getAttribute('Square150x150Logo'),
            )!;
            visualElements.square30x30Logo = this.fixupUrl(
                element.getAttribute('Square30x30Logo'),
            )!;
        }

        const defaultTileElement = element.getElementsByTagNameNS(
            AppX2013ManifestNS,
            'DefaultTile',
        )[0];
        if (defaultTileElement !== null) {
            visualElements.defaultTile = this.loadTextResources(
                this.readDefaultTile(defaultTileElement),
            );
        } else {
            visualElements.defaultTile = {
                shortName: visualElements.displayName,
            };
        }

        const splashScreenElement = element.getElementsByTagNameNS(
            AppX2013ManifestNS,
            'SplashScreen',
        )[0]!;
        visualElements.splashScreen =
            this.readSplashScreen(splashScreenElement);
    ...
    private readDefaultTile(element: Element): ApplicationDefaultTile {
        let defaultTile: Partial<ApplicationDefaultTile> = {
            showNameOnTiles: [],
        };
        defaultTile.shortName = element.getAttribute('ShortName')!;

        if (this.compatibilityMode == PackageCompatibilityMode.windows80) {
            defaultTile.wide310x150Logo = this.fixupUrl(
                element.getAttribute('WideLogo'),
            )!;
            ...
            defaultTile.square70x70Logo = this.fixupUrl(
                element.getAttribute('Square70x70Logo'),
            )!;
            defaultTile.wide310x150Logo = this.fixupUrl(
                element.getAttribute('Wide310x150Logo'),
            )!;
            defaultTile.square310x310Logo = this.fixupUrl(
                element.getAttribute('Square310x310Logo'),
            )!;

            const showOnElements = element.getElementsByTagNameNS(
                AppX2013ManifestNS,
                'ShowOn',
            );
            for (let i = 0; i < showOnElements.length; i++) {
                ...
                defaultTile.showNameOnTiles!.push(
                    tile.substring(0, tile.length - 4),
                );
            }
        }

        let tileUpdate = element.getElementsByTagNameNS(
            AppX2013ManifestNS,
            'TileUpdate',
        )[0];
        if (tileUpdate) {
