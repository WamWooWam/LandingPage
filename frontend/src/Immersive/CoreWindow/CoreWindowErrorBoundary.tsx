...
const CoreWindowErrorRenderer = ({ error }: CoreWindowErrorBoundaryProps) => {
    const isConnectionError =
        (error.message &&
            (error.message.match(/Failed to fetch/) ||
                error.message.match(/Loading CSS chunk/))) ||
        !navigator.onLine;
            ...
            <div id="contentContainer" class="mainContent">
                {isConnectionError ? (
                    <div id="mainTitle" class="title">
                        You aren&rsquo;t connected
                    </div>
                ) : (
                    <div id="mainTitle" class="title">
                        This page can&rsquo;t be displayed
                    </div>
                )}
                <div class="taskSection" id="taskSection">
                    {isConnectionError ? (
                        <ul id="notConnectedTasks" class="tasks">
                            ...
                            <li id="task2-2">
                                Verify that flight mode is turned off.
                            </li>
                            <li id="task2-3">
                                Make sure that your wireless switch is
                                turned on.
                            </li>
                            <li id="task2-4">
                                See if you can connect to mobile
                                broadband.
                            </li>
                        ...
                        <ul id="cantDisplayTasks" class="tasks">
                            <li id="task1-1">
                                ...
                                <span id="webpage" class="webpageURL">
...
export default class CoreWindowErrorBoundary extends Component<
    CoreWindowErrorBoundaryProps,
    { error?: Error }
> {
    ...
    static getDerivedStateFromProps(
        props: Readonly<CoreWindowErrorBoundaryProps>,
    ...
    render() {
        return (<div> {this.state.error ? <CoreWindowErrorRenderer error={this.state.error}/> : this.props.children} </div>)
    }
}
