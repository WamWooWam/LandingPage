...
export function getTileSizeForTemplate(template: string): TileSize {
    // Windows 8.1 types
    if (template.startsWith('TileWide310x150')) return TileSize.wide310x150;
    ...
    // Windows 8.0 types
    if (template.startsWith('TileWide')) return TileSize.wide310x150;
    else if (template.startsWith('TileSquare')) return TileSize.square150x150;
...
export function getVisuals(doc: Document, size: TileSize): TileVisual[] {
    ...
    for (const visualElement of visualElements) {
        ...
        for (let i = 0; i < bindingElements.length; i++) {
            ...
            if (
                getTileSizeForTemplate(template) !== size ||
                (fallback && getTileSizeForTemplate(fallback) !== size)
            )
                continue;

            let visual: TileVisual = {
                bindings: [],
                branding: <any>branding || 'logo',
            };
