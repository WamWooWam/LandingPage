...
export default class CoreWindowLayoutManager {
    ...
    constructor() {
        if (typeof ResizeObserver !== 'undefined') {
            new ResizeObserver(() => this.recalculateLayout()).observe(
                document.body,
            );
        } else {
    ...
    public addWindowToLayout(
        ...
        if (
            this.leftWindow &&
            this.rightWindow &&
            sizePreference !== ViewSizePreference.default
        ) {
            return false;
        }
    ...
    public snapWindow(window: CoreWindow, snap: CoreWindowSnapState) {
        if (
            snap === CoreWindowSnapState.none ||
            snap === CoreWindowSnapState.full
        ) {
            this.leftWindow = window;
            this.rightWindow = null;
            this.state = CoreWindowLayoutKind.fullScreen;
    ...
    public removeWindowFromLayout(
        window: CoreWindow,
        doUnsnap: boolean = true,
    ): boolean {
        if (this.leftWindow !== window && this.rightWindow !== window) {
            return false;
        }
        ...
        if (
            doUnsnap ||
            (this.leftWindow === null && this.rightWindow === null)
        ) {
            this.state = CoreWindowLayoutKind.fullScreen;
        }
