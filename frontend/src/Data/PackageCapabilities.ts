...
export async function ensureCapabilitiesAsync(pack: Package): Promise<void> {
    ...
    let features = pack.capabilities.filter(
        (c) =>
            c.type === 'FeatureCapability' &&
            c.ns === 'https://wamwoowam.co.uk/appx/2022',
    );
...
async function ensureFeatureCapabilityAsync(
    feature: string,
    values: any,
): Promise<void> {
    if (feature === 'WebAssembly') {
        ...
        for (const feature of values['Features'].split(',')) {
            ...
            console.log(
                `Package required feature ${feature} is ${hasFeature ? 'supported' : 'not supported'}`,
            );
...
async function ensureWebAssemblyFeatureAsync(
    feature: string,
): Promise<boolean> {
    switch (feature) {
        case 'SIMD':
