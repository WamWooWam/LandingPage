...
import { CalcManager, calc_display_t, calc_manager_t, calc_resource_provider_t } from "./CalcManager/CalcManager";
...
export default class CalculatorModel {
    ...
    constructor(root: typeof CalcManager) {
        ...
        this.resourceLoader = root.calc_resource_provider_create({
            get_cengine_string: (id: string) => {
                switch (id) {
                    ...
                    default:
                        if (!CalcManagerStrings.hasOwnProperty(id))
                            return id;
                        return CalcManagerStrings[id as keyof typeof CalcManagerStrings];
                }
            }
        });
        ...
        this.manager = root.calc_manager_create(this.display, this.resourceLoader);
    }
    ...
    onKeyUp(event: KeyboardEvent) {
        ...
        const command = keyCommandMap[event.key.toLowerCase() as keyof typeof keyCommandMap];
        if (command !== undefined) {
            this.root.calc_manager_send_command(this.manager, command);
        }
    }
