...
async function latestPosts(req: Request, res: Response) {
    ...
    for (const data of posts) {
        ...
        if (data.post.embed?.$type === 'app.bsky.embed.images#view') {
            ...
            largeContent.getElementsByTagName('image')[0].setAttribute('src', embed.images[0].thumb);
            largeContent.getElementsByTagName('image')[0].setAttribute('alt', embed.images[0].alt);
