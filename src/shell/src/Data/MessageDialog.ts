...
export default class MessageDialog {
    ...
    showAsync(): Promise<UICommand> {
        ...
        return new Promise((resolve, reject) => {
            ...
            events.addEventListener("message-dialog-close", (e: MessageDialogEvent) => {
                if (e.detail.dialog.id !== this.id)
                    return;

                if (e.detail.command)
                    resolve(e.detail.command);
                else
                    reject();
            })

            events.dispatchEvent(new MessageDialogEvent({ type: "open", dialog: this }))
        });
    }
}
